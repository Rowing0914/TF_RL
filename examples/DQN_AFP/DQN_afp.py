...
DQN with Ask for Help of Expert
...
from tf_rl.common.train import train_DQN_afp
...
if __name__ == '__main__':
	logdirs = logdirs()
	try:
		os.system("rm -rf {}".format(logdirs.log_DQN_afp))
	except:
		pass
	...
	agent = DQN(args.mode, Model, Model, env.action_space.n, params, logdirs.model_DQN_afp)
	if params.policy_fn == "Eps":
		Epsilon = AnnealingSchedule(start=params.epsilon_start, end=params.epsilon_end,
									decay_steps=params.decay_steps)
		agent_policy = EpsilonGreedyPolicy_eager(Epsilon_fn=Epsilon)
	elif params.policy_fn == "Boltzmann":
		agent_policy = BoltzmannQPolicy_eager()

	reward_buffer = deque(maxlen=params.reward_buffer_ep)
	summary_writer = tf.contrib.summary.create_file_writer(logdirs.log_DQN_afp)

	expert = DQN(args.mode, Model, Model, env.action_space.n, params, logdirs.model_DQN)
	expert_policy = TestPolicy()
	expert.check_point.restore(expert.manager.latest_checkpoint)
	print("Restore the model from disk")

	train_DQN_afp(agent, expert, env, agent_policy, expert_policy, replay_buffer, reward_buffer, params, summary_writer)
