...
parser.add_argument("--loss_fn", default="huber", help="types of loss function => MSE or huber_loss")
...
parser.add_argument("--log_dir", default="../../logs/logs/Duelling_DQN/", help="directory for log")
parser.add_argument("--model_dir", default="../../logs/models/Duelling_DQN/", help="directory for trained model")
...
if params.mode == "CartPole":
	...
	agent = DQN_cartpole(Model, optimizer, loss_fn, grad_clip_fn, env.action_space.n, params.gamma, params.model_dir)
elif params.mode == "CartPole-p":
	env = CartPole_Pixel(gym.make("CartPole-v0"))
	params.log_dir = "../../logs/logs/" + now.strftime("%Y%m%d-%H%M%S") + "-Duelling_DQN-p/"
	params.model_dir = "../../logs/models/" + now.strftime("%Y%m%d-%H%M%S") + "-Duelling_DQN-p/"
	agent = DQN(Model_p, optimizer, loss_fn, grad_clip_fn, env.action_space.n, params.gamma, params.model_dir)
