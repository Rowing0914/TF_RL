...
reward_run, reward_ctrl = list(), list()
actions = list()
...
while not done:
    ...
    # Reward Calculation in gym/halfcheetah.py
...
reward_run, reward_ctrl = np.array(reward_run), np.array(reward_ctrl)
r = np.array(r)

plt.plot(range(len(r)), r, label="Reward per step")
plt.plot(range(len(reward_run)), reward_run, label="reward_run")
