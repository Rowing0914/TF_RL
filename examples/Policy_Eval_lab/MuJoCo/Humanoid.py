...
reward_linvel, reward_quadctrl, reward_alive, reward_impact = list(), list(), list(), list()
actions = list()
...
while not done:
    ...
    # Reward Calculation in gym/humanoid.py
    # reward = lin_vel_cost - quad_ctrl_cost - quad_impact_cost + alive_bonus
    r.append(reward)
    actions.append(action)
    reward_linvel.append(info["reward_linvel"])
    reward_quadctrl.append(info["reward_quadctrl"])
    reward_alive.append(info["reward_alive"])
    reward_impact.append(info["reward_impact"])

env.close()
print("Ep Reward: ", episode_reward)

reward_linvel, reward_quadctrl, reward_alive, reward_impact = np.array(reward_linvel), np.array(reward_quadctrl), np.array(reward_alive), np.array(reward_impact)
r = np.array(r)

plt.plot(range(len(r)), r, label="Reward per step")
plt.plot(range(len(reward_linvel)), reward_linvel, label="reward_linvel")
plt.plot(range(len(reward_quadctrl)), reward_quadctrl, label="reward_quadctrl")
plt.plot(range(len(reward_alive)), reward_alive, label="reward_alive")
plt.plot(range(len(reward_impact)), reward_impact, label="reward_impact")
