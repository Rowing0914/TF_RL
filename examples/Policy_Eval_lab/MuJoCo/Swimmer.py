...
parser.add_argument("--env_name", default="Swimmer-v2", help="Env title")
...
reward_fwd, reward_ctrl = list(), list()
actions = list()
...
while not done:
    ...
    # Reward Calculation in gym/swimmer.py
    # reward = reward_ctrl + reward_fwd
    r.append(reward)
    actions.append(action)
    reward_fwd.append(info["reward_fwd"])
    reward_ctrl.append(info["reward_ctrl"])

env.close()
print("Ep Reward: ", episode_reward)

reward_fwd, reward_ctrl = np.array(reward_fwd), np.array(reward_ctrl)
r = np.array(r)

plt.plot(range(len(r)), r, label="Reward per step")
plt.plot(range(len(reward_fwd)), reward_fwd, label="reward_fwd")
