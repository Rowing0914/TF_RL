...
class _Duelling_Double_DQN_PER:
    ...
    def update(self, sess, state, action, Y):
        ...
        summaries, total_t, _, loss, batch_loss = sess.run(
            [self.summaries, tf.train.get_global_step(), self.train_op, self.loss, self.losses], feed_dict=feed_dict)
            ...
            if done:
                ...
                if frame_idx > params.learning_start and len(replay_buffer) > params.batch_size:
                    ...
                    states, actions, rewards, next_states, dones, weights, indices = replay_buffer.sample(
                        params.batch_size, Beta.get_value(frame_idx))
                    ...
                    losses.append(loss)
                    logging(frame_idx, params.num_frames, index_episode, time.time() - start, episode_reward,
