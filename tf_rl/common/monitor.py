...
Since the monitor class is lacking of some flexibility of switch on/off to record the video
I added two simple APIs

- record_start()
- record_end()

By alternating them, we can switch off/on the recording!!
...
class Monitor(Wrapper):
    def __init__(self, env, directory, force=False, resume=False,
        ...
        self._start(directory, force, resume, write_upon_reset, uid, mode)
    ...
    def _start(self, directory, force=False, resume=False,
              write_upon_reset=False, uid=None, mode=None):
        """Start monitoring.
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
    ...
    def record_start(self):
        """ Manually activate the recording func """
        self.video_flg = True
        self.enabled = True

    def record_end(self):
        """ Manually deactivate the recording func """
        self.reset()
        self.video_flg = False
        self.enabled = False
