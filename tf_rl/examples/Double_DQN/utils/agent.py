...
class Double_DQN(object):
    ...
    def _update(self, states, actions, rewards, next_states, dones):
        ...
        with tf.GradientTape() as tape:
            q_tp1_main = self.main_model(next_states)  # batch_size x num_action
            q_tp1 = self.target_model(next_states)  # batch_size x num_action
            q_t = self.main_model(states)  # batch_size x num_action
            idx_flattened = tf.range(0, tf.shape(q_tp1)[0]) * tf.shape(q_tp1)[1] + tf.cast(
                tf.math.argmax(q_tp1_main, axis=-1), dtype=tf.int32)

            # passing [-1] to tf.reshape means flatten the array
            # using tf.gather, associate Q-values with the executed actions
            chosen_next_q = tf.gather(tf.reshape(q_tp1, [-1]), idx_flattened)  # (batch_size,)

            Y = rewards + self._gamma * chosen_next_q * (1. - dones)  # (batch_size,)
